...
> **上级文档**：[tesla-1y-master-plan.md](./tesla-1y-master-plan.md)（一年总排期，Q1 是其中第一季）
> **每日细节**：[tesla-q1-m1-daily.md](./tesla-q1-m1-daily.md)（M1 逐日任务）
...
- 把 W1-11 每周日积累的八股条目**汇总去重**成 `interview-qa-q1.md`（≥ 30 条，中英各一版），补齐 §五 表格里 M7 那一行的架构取舍类题目

**验收**：
- [ ] 简历更新完毕，这个项目列在最上面
- [ ] 能对着 README 用英文讲 15 分钟不看稿
- [ ] `interview-qa-q1.md` ≥ 30 条，全部录过英文口述
- [ ] 投出第一份 Tesla / 类外企简历（**即使觉得没准备好也要投** —— 早期投递是为了拿真实追问反馈，不是为了拿 offer）

---

## 四、每周节奏

> Q1 是一年计划的第一季，节奏与 [tesla-1y-master-plan.md](./tesla-1y-master-plan.md) §二 保持一致。
> 与最初版本的差别：工作日晚**开场 30 分钟划给算法**，项目时段收窄到 90 分钟；周日多 30 分钟整理八股。

| 时间 | 时长 | 内容 |
|---|---|---|
| 工作日 21:00 – 21:30 | 30min × 5 | **算法**：1-2 题，走完整 8 步流程（见总排期 §八） |
| 工作日 21:30 – 23:00 | 90min × 5 | **项目**：当前里程碑开发 |
| 周六 09:00 – 12:00 | 3h | **项目**：本周最难那块（新概念 / 卡壳 / 需要连续思考的） |
| 周日 20:00 – 21:00 | 1h | **八股**：整理本周碰到的 3-5 个问题，中英各一版 |
| 周日 21:00 – 21:30 | 30min | **周报**（自己看）+ 下周任务拆解 |
| 每天通勤 | 20min × 10 | 影子跟读英文技术视频 |
| 每周 2 次 | 30min × 2 | iTalki 外教，专聊本周做的技术（每月至少 1 次讲算法题） |

**总投入**：主动学习约 17h / 周，碎片约 4h / 周。

**硬底线**：任何一周低于 10h，按此顺序砍 —— 先砍周六项目时长，再砍八股，**算法和 iTalki 不许砍**（这两个断了最难恢复）。低于 10h 连续 3 周则整体延期一个季度，不压缩内容。
...
| W7-8 | 栈 / 队列 | 12 |
| W9-10 | 链表 | 10 |
| W11-12 | 阶段复盘 + 重做所有做错的题 | 12 |

**总量约 70 题**，以 Easy / Easy-Medium 为主。语言用 **TypeScript**（先保证思路通畅，不为语言分心）。题单只开 LeetCode「Top Interview 150」，**不要开第二个**。

**每周至少 2 道全程出声用英文走完流程** —— 这条最容易偷懒，也最不能偷懒。

---

## 五、面试题储备（每周日分摊整理，不留到 Week 12）

**原则**：只整理**本周项目里真实碰到过**的问题。没做过的技术不进文档 —— 外企技术面是层层追问，背来的守不住第二层，做过的能撑到第四层。

季末汇总成 `interview-qa-q1.md`，目标 ≥ 30 条，中英各一版，英文版录音自听。

| 里程碑 | 该周整理的题目 |
|---|---|
| **M1**（W1-2） | Docker 网络与 volume 机制；数据库连接池是什么、池满了会怎样；Swagger / OpenAPI 的实际价值 |
| **M2**（W3-4） | JWT vs Session，什么时候**不**该用 JWT；access token 短 / refresh token 长的取舍依据；refresh token 存哪、为什么；bcrypt vs argon2 / scrypt；refresh token 轮换（rotation）是什么、为什么要做；token 存 localStorage vs httpOnly cookie；JWT 泄露了怎么办 |
| **M3**（W5-7） | OAuth2 四种模式各自场景；**为什么要 code 中转、不直接给 token**；PKCE 解决什么问题；OIDC 与 OAuth2 的关系；scope 怎么设计；client_secret 泄露怎么办 |
| **M4**（W8-9） | 如何防 CSRF、防 XSS；CORS 预检请求；前端 token 存储方案取舍；首屏性能怎么优化 |
| **M5**（W10） | HTTPS / TLS 握手过程；证书怎么签发和续期；反向代理的作用；环境变量与密钥管理（为什么不能进 git） |
| **M6**（W11） | 结构化日志与 traceId 怎么贯穿一次请求；**liveness vs readiness 探针的区别**；Metrics / Logs / Traces 的边界；上线发现 bug 怎么回滚；CI 里跑不跑集成测试、为什么 |
| **M7**（W12） | 补齐架构取舍类：为什么 PostgreSQL 不用 MongoDB；Redis 挂了服务能撑多久、降级方案；单机部署瓶颈在哪、怎么水平扩展；token 撤销 / 强制下线怎么设计；rate limit 用滑动窗口还是令牌桶。**并把全部条目做一次英文口述录音** |
...
| 算法挤压项目进度 | 算法**不许砍**，砍周六项目时长。项目延到 Q2 前两周补完 |
| 想临时加考 AWS / GCP 证书 | 不考。理由见总排期 §七；替代方案是 Q3 一次真实 AWS 部署 |

---

## 七、Q1 结束时的自检

五个问题，任何一个答"否"都说明 Q1 不算通过：

1. 我能不能不看代码，白板画出完整的 OAuth2 授权码 + PKCE 时序图？
2. 我能不能用英文给一个陌生工程师讲清楚这个项目的三个技术取舍？
3. 我的 GitHub 上有没有一个 star 数 ≥ 0 但**面试官愿意花 5 分钟点进去看**的仓库？
4. 算法是否累计 ≥ 70 题，且能**用英文**讲清任意 3 道？
5. `interview-qa-q1.md` 是否 ≥ 30 条，且每条都能追问到第二层？

如果都能答"是"，Q2 就有底气切 Go + 分布式。

**W12 检查点**（对应总排期 §九）：若项目未上线或算法 < 50 题 → Q2 砍掉 M10（消息队列），先把 Q1 补完，不要带着欠账进 Q2。
