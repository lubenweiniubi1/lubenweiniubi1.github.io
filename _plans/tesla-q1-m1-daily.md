...
> **原则**：每天任务控制在 1.5-2h，卡壳超过当天预算 → 简化 → 记录到"卡壳日志"，第二天不重试同一方案
...
### Day 7（周日）· 复盘 + Week 2 拆解

**任务**（约 1h）：
1. 写一份 Week 1 复盘（放进 `_plans/journal/week1.md`）：
   - 做完了什么
   - 卡壳在哪、怎么解决的
   - Week 2 要调整什么
2. 检查 Week 2 任务清单（下方），把每天的任务落到日历
3. **奖励自己休息半天**——别通宵，长期节奏比单周产出重要
...
**任务**（约 2h）：
1. 从 clean 状态跑一遍完整流程，录一个 3 分钟视频：
   - `git clone`
   - `docker compose up`
   - 访问 Swagger
   - 调一次 `/users/:id`
   - 展示 CI 全绿
2. 视频放进 README（或 GitHub Discussions）
3. 写 M1 复盘 `_plans/journal/m1-retro.md`：
   - 计划 vs 实际（哪些超时、哪些提前）
   - 学到的最重要 3 个知识点
   - M2（认证模块）需要提前准备什么
4. 简历"个人项目"栏加一行占位（"OAuth2 认证服务，进行中，M1 完成"）
...
**为什么记这个**：M7 面试题储备时，这个文件就是你最真实的素材库。
