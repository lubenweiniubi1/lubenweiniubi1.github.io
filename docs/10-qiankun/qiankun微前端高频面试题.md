...
## 二、import-html-entry：qiankun 是怎么"import"子应用的？

qiankun 的核心不是自己实现的加载器，而是依赖 [`import-html-entry`](https://github.com/kuitos/import-html-entry) 这个库。理解它的工作原理，是理解 qiankun 的关键。

### Q3.5: 为什么 qiankun 用 HTML Entry 而不是 JS Entry？

**回答要点：**

single-spa 早期用的是 **JS Entry**——直接暴露一个打包好的 JS 入口给主应用。这种方式的问题：

1. **子应用要把所有静态资源打成一个 JS 文件**，失去代码分割优势
2. **CSS 需要用 JS 动态注入**，静态资源变成运行时依赖
3. **改一个字符串就要重发**整个巨型 bundle
4. **子应用无法独立运行**，因为没有独立的 HTML 入口

qiankun 改用 **HTML Entry**：
- 子应用照常打包（含 `index.html` + 拆分好的 JS/CSS chunks）
- 主应用 fetch 子应用的 `index.html`，从中解析出所有 `<script>` 和 `<link>`
- 优势：**子应用改动小、能独立部署、支持代码分割**

### Q3.6: `import-html-entry` 的完整工作流程是怎样的？

**回答要点：**

```
① fetch('http://localhost:8001/')        拉子应用的 index.html
② 解析 HTML 字符串                        用正则 + processTpl 分离 template / scripts / styles
③ 并行拉取所有外链资源                    <link href="a.css"> → fetch → 内联；<script src="b.js"> → fetch 拿文本
④ 处理内联脚本（entry script）            标记 entry，其余先执行
⑤ 返回 { template, execScripts, ... }    template 是替换掉 script/link 后的 HTML 字符串
⑥ qiankun 把 template 插入 container      用户看到骨架
⑦ 调用 execScripts(sandbox)              在沙箱里 eval 所有 script → 拿到 entry 导出的生命周期
⑧ 调用 mount(props)                       子应用真正挂载
```

**关键源码简化版：**

```js
// 简化的 import-html-entry 主流程
async function importHTML(url) {
  const htmlText = await fetch(url).then(r => r.text());

  // 1. 用正则拆分 HTML
  const { template, scripts, styles, entry } = processTpl(htmlText, url);

  // 2. 并行拉取所有 <link> 和外链 <script>
  const [styleContents, scriptContents] = await Promise.all([
    Promise.all(styles.map(s => fetch(s).then(r => r.text()))),
    Promise.all(scripts.map(s => typeof s === 'string' ? fetch(s).then(r => r.text()) : s.content)),
  ]);

  return {
    template: replaceStyleAndScriptTags(template, styleContents),
    // 关键:返回一个函数,由外部决定何时以及在哪个沙箱中执行
    execScripts: async (proxy) => {
      let exports;
      scriptContents.forEach((code, i) => {
        // 用 with + Function 把 window 换成沙箱 proxy
        const wrappedCode = `
          (function(window, self, globalThis){
            with(window) { ${code} }
          }).bind(window.proxy)(window.proxy, window.proxy, window.proxy)
        `;
        const result = new Function('proxy', wrappedCode)(proxy);
        // entry 脚本的导出就是生命周期钩子
        if (scripts[i] === entry) exports = proxy[/* libraryName */];
      });
      return exports;                     // { bootstrap, mount, unmount }
    },
  };
}
```

### Q3.7: 为什么子应用必须打成 UMD？qiankun 是怎么拿到 `bootstrap` / `mount` / `unmount` 的？

**回答要点：**

qiankun 通过**约定的全局变量名**拿生命周期钩子。子应用打包为 UMD 后，会在执行时把导出挂到 `window[libraryName]`：

```js
// webpack.config.js
output: {
  library: `${packageName}-[name]`,      // 例:my-app-main
  libraryTarget: 'umd',                  // ⭐ 必须 UMD
  globalObject: 'window',                // 让 UMD 挂载目标是 window(而非 self,避免 Web Worker 环境错乱)
}
```

`import-html-entry` 在执行完 entry script 后，会尝试从 `sandbox.proxy` 上取出导出：

```
执行 UMD 脚本 → 匿名 IIFE 判断环境
  → 发现有 window → 挂 window[libraryName] = { bootstrap, mount, unmount }
  → 因为 window 被沙箱代理了,实际写入 fakeWindow[libraryName]
  → qiankun 从 fakeWindow[libraryName] 拿到生命周期
```

**如果没打成 UMD 会怎样？**
- ESM 格式:浏览器不认(用 `eval` 无法执行 ESM)
- CommonJS 格式:`module.exports` 在浏览器环境未定义
- IIFE 格式:导出的东西没挂到 window,qiankun 拿不到生命周期钩子

### Q3.8: 主应用 fetch 到 HTML 后,子应用里的 `<script src="./a.js">` 为什么能正确加载?

**回答要点:**

关键在 **`publicPath` 的动态设置**。假设:
- 主应用地址:`https://main.com/`
- 子应用打包时 `publicPath: '/'`
- 子应用真实地址:`https://sub.com:8001/`

如果什么都不做,子应用的 HTML 里写的是 `<script src="/static/js/main.js">`,主应用 fetch 到这段 HTML 后,浏览器会去 `https://main.com/static/js/main.js` 找——**404**。

qiankun / import-html-entry 做了两件事:

1. **fetch HTML 时把相对路径改成绝对路径**:`processTpl` 里遇到 `<script src="./a.js">` 会重写成 `<script src="http://localhost:8001/a.js">`。

2. **动态修改 webpack `__webpack_public_path__`**:子应用入口最前面加一段:
...
qiankun 会在加载时把子应用的真实 URL 注入到 `__INJECTED_PUBLIC_PATH_BY_QIANKUN__`。这样运行时 webpack 动态 import 出的 chunk (`import(/* webpackChunkName */ './Foo')`) 才能拉到正确的地址。

### Q3.9: import-html-entry 里的 `<script>` 是用什么方式执行的?为什么不用 `new Script()` 直接插入?

**回答要点:**

**方式:** 用 `eval` 或 `new Function` 执行文本。

**为什么不用 `<script>` 标签?** 因为要**在沙箱里执行**。

如果直接 `document.head.appendChild(scriptTag)`:
- 脚本在真实 window 上执行,污染全局
- 无法拦截对 `window.xxx` 的读写
- 卸载时无法回收

而用 `new Function('with(window){ ... }')`,配合传入的 `proxy` 作为 `window`,就能:
- 让 `window.foo = 1` 实际写到 `proxy.foo` 上
- `console.log(window.location)` 会读到 `proxy.location`
- 卸载时丢掉 `proxy` 就相当于清空所有影响

代价:**eval 的代码没有独立的 script 上下文**,`document.currentScript` 会是 null,一些库(如某些 CDN 版 SDK)会因此报错。

### Q3.10: import-html-entry 的资源缓存策略?为什么切来切去不重新加载?

**回答要点:**

`import-html-entry` 内部维护了两级缓存:

1. **`embedHTMLCache`**:整个 HTML 解析结果的缓存,key 是入口 URL
2. **单个资源缓存**:每个 CSS / JS 的文本内容,key 是资源 URL

```js
// 简化实现
const cache = {};
export function importEntry(entry, opts = {}) {
  if (cache[entry]) return cache[entry];    // 命中缓存
  return cache[entry] = importHTML(entry, opts);
}
```

这就是为什么第一次进子应用慢,之后来回切换很快——**HTML 和 JS 文本都在内存里**,只是重新在沙箱里 eval 一遍。

**副作用:** 子应用发布了新版本,只要主应用没刷新,用的还是旧代码。**解决方案**:入口 URL 带 hash / 版本号,或者用 `singular: false` + 强制清缓存。
